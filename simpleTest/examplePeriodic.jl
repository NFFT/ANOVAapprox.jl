...
function TestFunction(x::Vector{Float64})::Float64
    return sin(4*pi*x[1]*x[5]) + 2 - exp(sin(2*pi*x[4])) + sin(4*pi*x[6]*x[2])^2
end
...
y      = y .+ 0.0*im      # make the Function values complex since this is needed for periodic approximation
y_test = y_test .+ 0.0*im # same for the test values

##########################
## Do the approximation ##
##########################

ads = ANOVAapprox.approx(X, y, ds, bw, "per") # generate the data structure for the approximation
ANOVAapprox.approximate(ads, lambda = λs, solver = "fista")   # do the approximation for all specified regularisation parameters
...
println("important dimensional interactions: "*string(label[gsis.>1e-2]))

################################################
## Approximation with better suited index set ##
################################################

U    = ads.U[append!([true],gsis.>1e-2)] # get important subsets
...
a = ANOVAapprox.approx(X, y, U, N, "per") # generate the data structure for the approximation
ANOVAapprox.approximate(a, lambda = λs, solver = "fista")   # do the approximation for all specified regularisation parameters
