...
X      = rand(rng, d, M)             # construct the evaluation points for training
y      = [TestFunction(X[:, i]) for i = 1:M] # evaluate the function at these points
X_test = rand(rng, d, M_test)                 #
...
ads = ANOVAapprox.approx(X, y, ds, bw, "cos") # generate the data structure for the approximation
...
println("important dimensional interactions: "*string(label[gsis.>1e-5]))

################################################
## Approximation with better suited index set ##
################################################

U    = ads.U[append!([true],gsis.>1e-5)] # get important subsets
...
a = ANOVAapprox.approx(X, y, U, N, "cos") # generate the data structure for the approximation
...
y_eval_anova_4_plot = y_eval_anova_4[perm]
y_anova_4_plot = -exp.(X_plot) .+ exp(1) .- 1
